...
2. a) The file.size function in R gives the size of the file in bytes. So, the csv file is 133887710 bytes and the rda file is 80000087 bytes. Binary files have that numeric characters will be 8 bytes/ character while in csv, this is not always the case. Since RDA is a binary format, we can divide the bytes by 8 to get how many numberic characters there are in the file; there are about 100,00 numeric characters and the 87 other bytes store metadata about the file. The csv file, which also has 100,000 numbers, is bigger because each separate character is a byte in ASCII file formats, including commas and new lines. So, for example, this table has 100 columns so that means that the csv file needs to store 99 commas per row as well as new line characters. 
